...
void dump(const ui64 *leaks, ui32 count);
...
void
dump(const ui64 *leaks, ui32 count) {
    for (uint8_t i = 0; i < count; i++) {
        if (i * sizeof(uint64_t) >= count) {
            break;
        }
        uint64_t current_leak = leaks[i];
        char leak_str[99];
        sprintf(leak_str, "%#02lx", current_leak);
        printf("\t--> %d: buf + 0x%x\t: %s\n", i, (unsigned int)(sizeof(leaks[0]) * i), leak_str);
    }
}
